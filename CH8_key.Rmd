...
\begin{align*}
\frac{d RSS}{d \beta_0} & = - 2 \sum_{i=1}^{n} \left( y_i - \beta_0 - \beta_1 x_i\right)\\
\text{(set = 0)\;\;} - 2 \sum_{i=1}^{n} \left( y_i - \beta_0 - \beta_1 x_i\right)  & = 0\\
...
- $\vec{y}$ is a $n \times 1$ matrix (or vector), such that $$\vec{y} = \begin{pmatrix}
y_1 \\
y_2\\
\vdots \\
y_n
...
- $\vec{\beta}$ is $2 \times 1$ matrix, $\vec{\beta} = (\beta_0 \;\; \beta_1)$

\vfill

- $\vec{\epsilon}$ is a $n \times 1$ matrix (or vector), such that $$\vec{\epsilon} = 
\begin{pmatrix}
\epsilon_1\\
\epsilon_2 \\
\vdots \\
...
With least-squares estimation, there is no specification of a probability distribution. This is strictly a geometric procedure. Nevertheless, the result is the same as what is known as the _maximum likelihood_ estimates.
...
The authors suggest calling the intervals either "uncertainty intervals" or "compatibility intervals".
